...
SpeechBubble {
	...
	bubbleContent: Label {
		...
		text: from_me==20 ? qsTr("<b>%1</b> has join the group").arg(getAuthor(message)) :
			  ...
			  from_me==22 ? qsTr("<b>%1</b> has changed the subject to <b>%2</b>").arg(name).arg(message) : message
		color: from_me==1 ? "black" : from_me==0 ? "white" : "gray"
		width: (appWindow.inPortrait ? 380 : 754)
		wrapMode: "WrapAtWordBoundaryOrAnywhere"
		anchors.left: parent.left
		anchors.leftMargin: from_me==20||from_me==21||from_me==22? 50 : from_me ? 20 : 80
		textFormat: Text.RichText
		font.family: "Nokia Pure Light"
		font.weight: Font.Light
		font.pixelSize: from_me==20||from_me==21||from_me==22 ? 18 : 23
		horizontalAlignment: from_me==20||from_me==21||from_me==22? Text.AlignHCenter : from_me==1? Text.AlignLeft : Text.AlignRight
